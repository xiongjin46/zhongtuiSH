...
public class CurrentProjectFragment extends BaseFragment {
    ...
    public static CurrentProjectFragment newInstance(int type) {
    ...
    private void onRefresh(int page) {
        if (!isload) {
            ...
            userEngine.getCurrentTaskInfo(id, page);
        }
    }
    ...
    class UserSub extends UserCallback.Stub {
        ...
        public void onGetCurrentTaskSuccess(HistoryTaskResp rsp) {
            super.onGetCurrentTaskSuccess(rsp);
            isload = false;
            if (frame != null)
        ...
        public void onGetCurrentTaskFail(int errCode, String msg) {
            super.onGetCurrentTaskFail(errCode, msg);
            ((BaseToolbarActivity) mActivity).dissProgressDlg();
            isload = false;
            if (frame != null)
                frame.refreshComplete();
