...
import com.push.PushMerchant.view.ArcLayout;
 ...
 * @Description: 自定翻页的ViewPager控件
 * @author: YYL
 * <p/>
 * create at 2016/3/9 14:52
 */
public class ConvenientBanner<T> extends LinearLayout {
    ...
    private void init(Context context) {
        ...
        arclayout = (ArcLayout) hView.findViewById(R.id.arclayout);
    ...
    public void setArclayout(float acrHeight,boolean cropInside,int position){
        arclayout.setArcLayout(acrHeight,cropInside,position);
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (this.mView != null && ev.getAction() != MotionEvent.ACTION_UP) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                moveY = Math.abs(ev.getY() - oldY);
                moveX = Math.abs(ev.getX() - oldX);
                if (moveX > 20) {
                    startScroll = false;
                }
                break;
            case MotionEvent.ACTION_UP:
                moveY = moveX = oldX = oldY = 0;
                startScroll = true;
                break;
        }
    ...
    public boolean isVertical() {
        return moveY >= moveX && startScroll;
    }
     ...
     */
    public ConvenientBanner setOnItemClickListener(IOnItemClickListener onItemClickListener) {
